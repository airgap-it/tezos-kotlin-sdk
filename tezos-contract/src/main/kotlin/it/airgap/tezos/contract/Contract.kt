...
public class Contract(
    ...
    private val rpcCodeCached: Cached<RpcContractCode> = Cached { headers -> rpc.script.normalized.post(RpcScriptParsing.OptimizedLegacy, headers).toRpcContractCode() }
    ...
    private val metaEntrypointsCached: Cached<Map<String, MetaContractEntrypoint>> = Cached { headers -> rpc.entrypoints.get(headers).toMetaContractEntrypoint() }

    public val storage: ContractStorage by lazy { ContractStorage(metaStorageCached, rpc) }
    ...
    private fun RpcContractCode.toMetaContractStorage(): MetaContractStorage {
        ...
        return MetaContractStorage(type, michelineToCompactStringConverter)
    }
