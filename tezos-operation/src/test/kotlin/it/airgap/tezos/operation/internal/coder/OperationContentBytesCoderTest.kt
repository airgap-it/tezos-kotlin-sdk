package it.airgap.tezos.operation.internal.coder

import io.mockk.MockKAnnotations
import io.mockk.every
import io.mockk.impl.annotations.MockK
import io.mockk.mockkStatic
import io.mockk.unmockkAll
import it.airgap.tezos.core.internal.base58.Base58
import it.airgap.tezos.core.internal.base58.Base58Check
import it.airgap.tezos.core.internal.coder.*
import it.airgap.tezos.core.internal.crypto.Crypto
import it.airgap.tezos.core.internal.utils.asHexString
import it.airgap.tezos.michelson.decodeFromBytes
import it.airgap.tezos.michelson.encodeToBytes
import it.airgap.tezos.michelson.micheline.MichelineNode
import it.airgap.tezos.michelson.micheline.MichelineSequence
import it.airgap.tezos.operation.OperationContent
import it.airgap.tezos.operation.contract.Entrypoint
import it.airgap.tezos.operation.contract.Parameters
import it.airgap.tezos.operation.contract.Script
import it.airgap.tezos.operation.header.Fitness
import it.airgap.tezos.operation.header.FullHeader
import it.airgap.tezos.operation.inlined.InlinedEndorsement
import it.airgap.tezos.operation.inlined.InlinedPreendorsement
import it.airgap.tezos.operation.internal.converter.TagToOperationContentKindConverter
import org.junit.After
import org.junit.Before
import org.junit.Test
import java.security.MessageDigest
import kotlin.test.assertContentEquals
import kotlin.test.assertEquals
import kotlin.test.assertFailsWith

class OperationContentBytesCoderTest {

    @MockK
    private lateinit var crypto: Crypto

    private lateinit var operationContentBytesCoder: OperationContentBytesCoder

    @Before
    fun setup() {
        MockKAnnotations.init(this)

        every { crypto.hashSha256(any<ByteArray>()) } answers {
            val messageDigest = MessageDigest.getInstance("SHA-256")
            messageDigest.digest(firstArg())
        }

        mockkStatic(MichelineNode::encodeToBytes)
        mockkStatic(MichelineNode.Companion::decodeFromBytes)

        val base58Check = Base58Check(Base58(), crypto)

        val base58BytesCoder = Base58BytesCoder(base58Check)
        val keyHashBytesCoder = KeyHashBytesCoder(base58BytesCoder)
        val keyBytesCoder = KeyBytesCoder(base58BytesCoder)
        val signatureBytesCoder = SignatureBytesCoder(base58BytesCoder)
        val addressBytesCoder = AddressBytesCoder(keyHashBytesCoder, base58BytesCoder)
        val zarithNaturalNumberBytesCoder = ZarithNaturalNumberBytesCoder()

        val tagToOperationContentKindConverter = TagToOperationContentKindConverter()

        operationContentBytesCoder = OperationContentBytesCoder(
            base58BytesCoder,
            addressBytesCoder,
            keyBytesCoder,
            keyHashBytesCoder,
            signatureBytesCoder,
            zarithNaturalNumberBytesCoder,
            tagToOperationContentKindConverter,
        )
    }

    @After
    fun clean() {
        unmockkAll()
    }

    @Test
    fun `should encode Operation to bytes`() {
        every { MichelineSequence().encodeToBytes() } returns byteArrayOf(0)

        operationsWithBytes.forEach {
            assertContentEquals(it.second, operationContentBytesCoder.encode(it.first))
        }
    }

    @Test
    fun `should decode Operation from bytes`() {
        every { MichelineNode.decodeFromBytes(any()) } returns MichelineSequence()

        operationsWithBytes.forEach {
            assertEquals(it.first, operationContentBytesCoder.decode(it.second))
            assertEquals(it.first, operationContentBytesCoder.decodeConsuming(it.second.toMutableList()))
        }
    }

    @Test
    fun `should fail to decode Operation from invalid bytes`() {
        invalidBytes.forEach {
            assertFailsWith<IllegalArgumentException> {
                operationContentBytesCoder.decode(it)
            }

            assertFailsWith<IllegalArgumentException> {
                operationContentBytesCoder.decodeConsuming(it.toMutableList())
            }
        }
    }

    private val operationsWithBytes: List<Pair<OperationContent, ByteArray>>
        get() = listOf(
            OperationContent.SeedNonceRevelation(
                1,
                "6cdaf9367e551995a670a5c642a9396290f8c9d17e6bc3c1555bfaa910d92214"
            ) to "01000000016cdaf9367e551995a670a5c642a9396290f8c9d17e6bc3c1555bfaa910d92214".asHexString().toByteArray(),
            OperationContent.DoubleEndorsementEvidence(
                InlinedEndorsement(
                    "BLT3XKN3vFqWnWfuuLenQiyVgEgKcJttnGGdCcQbmE95xz9y7S5",
                    OperationContent.Endorsement(
                        1U,
                        1,
                        1,
                        "vh2cHpyeaHQhF7g3RFh8usyYmTTpt882UsRyXECuBwPiB3TcsKNd",
                    ),
                    "sigdV5DNZRBLBDDEkbWcqefBuMZevanVyjotoazkkLbk7jXR8oZUmnxt6n3hkQtTe9WbLEkcCUWw1Ey7Ybby5z35nHKqpndn"
                ),
                InlinedEndorsement(
                    "BLZS5mP4BufHrZfvzrvw1ReWnj1L2zcQ4mM6Jywoaxe4mHbiCNn",
                    OperationContent.Endorsement(
                        2U,
                        2,
                        2,
                        "vh2rXj5TAG8p1HKiMyaWDdYrRL2rTBPyFLkVorgzEEBqqd4sgsXG",
                    ),
                    "sigff9imsFxGwyQ8nEpXUR8ZFwTqZWjMJAgKGwub6Mn9Cnu4VvBppTRt84VPp1fRwqpx8JTrLHg76guTGzkm9ETKwFNCzniY"
                ),
            ) to "".asHexString().toByteArray(),
            OperationContent.DoubleBakingEvidence(
                FullHeader(
                    1,
                    1U,
                    "BKsP8FYgikDmqbUiVxfgXVjWuay5LQZY6LP4EvcsFK8uuqj4wQD",
                    1,
                    1U,
                    "op7BauymKmWRrNrWTsS9yCbE7wLbsxjtdTGvFzLZuMUzRmUe4Pd",
                    emptyList(),
                    "CoWKSZnE72uMLBeh3Fmj3LSXjJmeCEmYBMxAig15g3LPjTP4rHmR",
                    "vh2cJrNF6FCXo1bfnM9hj66NDQSGQCBxTtqkxkMLzkTeeDnZjrvD",
                    1,
                    "d4d34b5686c98ae1",
                    null,
                    true,
                    "sigiaEd9dHEGKgccx3JBBDw4eb6WVxGH3MvyziYbQqWQRMmyecdo5VuSkYWkgZvcQXshB4vV2qkTb6AxbKruaNPfnMg4u2EA"
                ),
                FullHeader(
                    2,
                    2U,
                    "BMaBxGyVhtTiMKd7KA8HXJnbTK4e1TzffNc94G18op55HGQYVRk",
                    2,
                    2U,
                    "oo1HUR2PPAShbHfQ2w4dFoywHSCJTX32Evwgc39zKaiN92ZzP3o",
                    emptyList(),
                    "CoVj5HxwnPHpC1SgCC6pgqVPgw2vqFEqaC2bF5STqcbyX6giVrGn",
                    "vh2MHqgJtw8v7CDrZKYWtLmqGJtjzkRvs9yUeHNQqdgDJyCYm21q",
                    2,
                    "336ebf95efce0475",
                    "nceUeUCJRZ4M7FCSBsAUZU6dmxePdH7irje9Gfj9zWwCdfWd5B4Ee",
                    false,
                    "sigRsUhHqaFVBeV4qzyCZ6Y9TvoKajyNwyPQQCW3SbgPYY99MrpTqR2FopjzZEHMWoJG7LaTaHu7bnieKQRKqCRLA7hB7Ekp"
                ),
            ) to "".asHexString().toByteArray(),
            OperationContent.DoubleBakingEvidence(
                FullHeader(
                    1,
                    1U,
                    "BKsP8FYgikDmqbUiVxfgXVjWuay5LQZY6LP4EvcsFK8uuqj4wQD",
                    1,
                    1U,
                    "op7BauymKmWRrNrWTsS9yCbE7wLbsxjtdTGvFzLZuMUzRmUe4Pd",
                    listOf(Fitness(1, null, 1, 1)),
                    "CoWKSZnE72uMLBeh3Fmj3LSXjJmeCEmYBMxAig15g3LPjTP4rHmR",
                    "vh2cJrNF6FCXo1bfnM9hj66NDQSGQCBxTtqkxkMLzkTeeDnZjrvD",
                    1,
                    "d4d34b5686c98ae1",
                    null,
                    true,
                    "sigiaEd9dHEGKgccx3JBBDw4eb6WVxGH3MvyziYbQqWQRMmyecdo5VuSkYWkgZvcQXshB4vV2qkTb6AxbKruaNPfnMg4u2EA"
                ),
                FullHeader(
                    2,
                    2U,
                    "BMaBxGyVhtTiMKd7KA8HXJnbTK4e1TzffNc94G18op55HGQYVRk",
                    2,
                    2U,
                    "oo1HUR2PPAShbHfQ2w4dFoywHSCJTX32Evwgc39zKaiN92ZzP3o",
                    listOf(Fitness(2, 2, 2, 2), Fitness(2, null, 2, 2)),
                    "CoVj5HxwnPHpC1SgCC6pgqVPgw2vqFEqaC2bF5STqcbyX6giVrGn",
                    "vh2MHqgJtw8v7CDrZKYWtLmqGJtjzkRvs9yUeHNQqdgDJyCYm21q",
                    2,
                    "336ebf95efce0475",
                    "nceUeUCJRZ4M7FCSBsAUZU6dmxePdH7irje9Gfj9zWwCdfWd5B4Ee",
                    false,
                    "sigRsUhHqaFVBeV4qzyCZ6Y9TvoKajyNwyPQQCW3SbgPYY99MrpTqR2FopjzZEHMWoJG7LaTaHu7bnieKQRKqCRLA7hB7Ekp"
                ),
            ) to "".asHexString().toByteArray(),
            OperationContent.ActivateAccount(
                "tz1PokEhtiBGCmekQrcN87pCDmqy99TjaLuN",
                "7b27ba02550e6834b50173c8c506de42d901c606",
            ) to "042db6ed2d71e8f22ce348c1b7b2e7f08892bd50ef7b27ba02550e6834b50173c8c506de42d901c606".asHexString().toByteArray(),
            OperationContent.Proposals(
                "tz1QVzD6eV73LhtzhNKs94fKbvTg7VjKjEcE",
                1,
                listOf("PtYnGfhwjiRjtA7VZriogYL6nwFgaAL9ZuVWE6UahXCMn6BoJPv"),
            ) to "050035533a79b20d6ea4dc8b92ab1cf33b448b93c78f0000000100000020f0e14a6c55f809a0ac08dc9bba0596b0daac1944520dfa9b8e2ce4e1a102a203".asHexString().toByteArray(),
            OperationContent.Proposals(
                "tz1MRrtJC9sk1o1D57LPWev6DDVjMgra5pXb",
                1,
                listOf("PtARwRL7jEGtzoCCWDBXe6ZJ4ZJiWtDgBC2a5WwnHYYyKPdmwrb", "Ps6NdX1CpeF3kHV5CVQZMLZKZwAN8NYN9crdL3GzEg4uNg7f3DY"),
            ) to "050013a312c56ed0eb53799ce6ef3eabfc1102f73b940000000100000040be2160f0cad3ca52a8e1a2f9e6fb25e748a769267dad2964550e6d946d0a03c23138e6c4f4e5e47064cbd0cd36b2a09ad01d0709b9737b1f8622a43448de01d5".asHexString().toByteArray(),
            OperationContent.Ballot(
                "tz1eNhmMTYsti2quW46a5CBJbs4Fde4KGg4F",
                1,
                "PsjL76mH8vo3fTfUN4qKrdkPvRfXw7KJPWf87isNAxzh1vqdFQv",
                OperationContent.Ballot.BallotType.Yay,
            ) to "0600cd8459db8668d3ae6a4f49cb8fe3c5bbd6c76956000000018522ef9f87cef2f745984cdbfe4a723acfbe7979c6f24ebc04a86d786b1c038500".asHexString().toByteArray(),
            OperationContent.Ballot(
                "tz1eNhmMTYsti2quW46a5CBJbs4Fde4KGg4F",
                1,
                "PsjL76mH8vo3fTfUN4qKrdkPvRfXw7KJPWf87isNAxzh1vqdFQv",
                OperationContent.Ballot.BallotType.Nay,
            ) to "0600cd8459db8668d3ae6a4f49cb8fe3c5bbd6c76956000000018522ef9f87cef2f745984cdbfe4a723acfbe7979c6f24ebc04a86d786b1c038501".asHexString().toByteArray(),
            OperationContent.Ballot(
                "tz1eNhmMTYsti2quW46a5CBJbs4Fde4KGg4F",
                1,
                "PsjL76mH8vo3fTfUN4qKrdkPvRfXw7KJPWf87isNAxzh1vqdFQv",
                OperationContent.Ballot.BallotType.Pass,
            ) to "0600cd8459db8668d3ae6a4f49cb8fe3c5bbd6c76956000000018522ef9f87cef2f745984cdbfe4a723acfbe7979c6f24ebc04a86d786b1c038502".asHexString().toByteArray(),
            OperationContent.DoublePreendorsementEvidence(
                InlinedPreendorsement(
                    "BLT3XKN3vFqWnWfuuLenQiyVgEgKcJttnGGdCcQbmE95xz9y7S5",
                    OperationContent.Preendorsement(
                        1U,
                        1,
                        1,
                        "vh2cHpyeaHQhF7g3RFh8usyYmTTpt882UsRyXECuBwPiB3TcsKNd",
                    ),
                    "sigdV5DNZRBLBDDEkbWcqefBuMZevanVyjotoazkkLbk7jXR8oZUmnxt6n3hkQtTe9WbLEkcCUWw1Ey7Ybby5z35nHKqpndn"
                ),
                InlinedPreendorsement(
                    "BLZS5mP4BufHrZfvzrvw1ReWnj1L2zcQ4mM6Jywoaxe4mHbiCNn",
                    OperationContent.Preendorsement(
                        2U,
                        2,
                        2,
                        "vh2rXj5TAG8p1HKiMyaWDdYrRL2rTBPyFLkVorgzEEBqqd4sgsXG",
                    ),
                    "sigff9imsFxGwyQ8nEpXUR8ZFwTqZWjMJAgKGwub6Mn9Cnu4VvBppTRt84VPp1fRwqpx8JTrLHg76guTGzkm9ETKwFNCzniY"
                ),
            ) to "".asHexString().toByteArray(),
            OperationContent.FailingNoop("cc7e647be422e432a3291ec8a2ee6f5e2210c51825b753758a99e266a0c65b15") to "1100000020cc7e647be422e432a3291ec8a2ee6f5e2210c51825b753758a99e266a0c65b15".asHexString().toByteArray(),
            OperationContent.Preendorsement(
                1U,
                1,
                1,
                "vh2KDvhtt44Lyq187SnZjSDyRH1LNXbMj3T9G57miWK9QvqH3fhv",
            ) to "1400010000000100000001543d9791df12f3237de836314a45a348e5d608c80a6a411246dfc67ef1a08d0a".asHexString().toByteArray(),
            OperationContent.Endorsement(
                1U,
                1,
                1,
                "vh2WtVuY9PK3mDsnfdzA6iXc4pocgUff8hgamWwXw19r5kDYHVS5",
            ) to "15000100000001000000016eba3d57f131a71eab0692e333e889cbafe523c675e588ace92bb5056cbcb889".asHexString().toByteArray(),
            OperationContent.Reveal(
                "tz1SZ2CmbQB7MMXgcMSmyyVXpya1rkb9UGUE",
                "135675",
                "154",
                "23675",
                "34152",
                "edpkuaARNJPQygG82X1xed6Z2kDutT8XjL3Fmv1XPBbca1uARirj55",
            ) to "6b004bd66485632a18d61068fc940772dec8add5ff93fba3089a01fbb801e88a02007a79d89acb296dd9ec2be8fba817702dc41adf19e28bb250a337f840eb263c69".asHexString().toByteArray(),
            OperationContent.Transaction(
                "tz1i8xLzLPQHknc5jmeFc3qxijar2HLG2W4Z",
                "135675",
                "154",
                "23675",
                "34152",
                "763243",
                "tz1YbTdYqmpLatAqLb1sm67qqXMXyRB3UYiz",
            ) to "6c00f6cb338e136f281d17a2657437f090daf84b42affba3089a01fbb801e88a02ebca2e00008e1d34730fcd7e8282b0efe7b09b3c57543e59c800".asHexString().toByteArray(),
            OperationContent.Transaction(
                "tz1i8xLzLPQHknc5jmeFc3qxijar2HLG2W4Z",
                "135675",
                "154",
                "23675",
                "34152",
                "763243",
                "tz1YbTdYqmpLatAqLb1sm67qqXMXyRB3UYiz",
                Parameters(Entrypoint.Default, MichelineSequence()),
            ) to "6c00f6cb338e136f281d17a2657437f090daf84b42affba3089a01fbb801e88a02ebca2e00008e1d34730fcd7e8282b0efe7b09b3c57543e59c8ff000000000100".asHexString().toByteArray(),
            OperationContent.Transaction(
                "tz1i8xLzLPQHknc5jmeFc3qxijar2HLG2W4Z",
                "135675",
                "154",
                "23675",
                "34152",
                "763243",
                "tz1YbTdYqmpLatAqLb1sm67qqXMXyRB3UYiz",
                Parameters(Entrypoint.Root, MichelineSequence()),
            ) to "6c00f6cb338e136f281d17a2657437f090daf84b42affba3089a01fbb801e88a02ebca2e00008e1d34730fcd7e8282b0efe7b09b3c57543e59c8ff010000000100".asHexString().toByteArray(),
            OperationContent.Transaction(
                "tz1i8xLzLPQHknc5jmeFc3qxijar2HLG2W4Z",
                "135675",
                "154",
                "23675",
                "34152",
                "763243",
                "tz1YbTdYqmpLatAqLb1sm67qqXMXyRB3UYiz",
                Parameters(Entrypoint.Do, MichelineSequence()),
            ) to "6c00f6cb338e136f281d17a2657437f090daf84b42affba3089a01fbb801e88a02ebca2e00008e1d34730fcd7e8282b0efe7b09b3c57543e59c8ff020000000100".asHexString().toByteArray(),
            OperationContent.Transaction(
                "tz1i8xLzLPQHknc5jmeFc3qxijar2HLG2W4Z",
                "135675",
                "154",
                "23675",
                "34152",
                "763243",
                "tz1YbTdYqmpLatAqLb1sm67qqXMXyRB3UYiz",
                Parameters(Entrypoint.SetDelegate, MichelineSequence()),
            ) to "6c00f6cb338e136f281d17a2657437f090daf84b42affba3089a01fbb801e88a02ebca2e00008e1d34730fcd7e8282b0efe7b09b3c57543e59c8ff030000000100".asHexString().toByteArray(),
            OperationContent.Transaction(
                "tz1i8xLzLPQHknc5jmeFc3qxijar2HLG2W4Z",
                "135675",
                "154",
                "23675",
                "34152",
                "763243",
                "tz1YbTdYqmpLatAqLb1sm67qqXMXyRB3UYiz",
                Parameters(Entrypoint.RemoveDelegate, MichelineSequence()),
            ) to "6c00f6cb338e136f281d17a2657437f090daf84b42affba3089a01fbb801e88a02ebca2e00008e1d34730fcd7e8282b0efe7b09b3c57543e59c8ff040000000100".asHexString().toByteArray(),
            OperationContent.Transaction(
                "tz1i8xLzLPQHknc5jmeFc3qxijar2HLG2W4Z",
                "135675",
                "154",
                "23675",
                "34152",
                "763243",
                "tz1YbTdYqmpLatAqLb1sm67qqXMXyRB3UYiz",
                Parameters(Entrypoint.Named("named"), MichelineSequence()),
            ) to "6c00f6cb338e136f281d17a2657437f090daf84b42affba3089a01fbb801e88a02ebca2e00008e1d34730fcd7e8282b0efe7b09b3c57543e59c8ffff056e616d65640000000100".asHexString().toByteArray(),
            OperationContent.Origination(
                "tz1LdF7qHCJg8Efa6Cx4LZrRPkvbh61H8tZq",
                "135675",
                "154",
                "23675",
                "34152",
                "763243",
                null,
                Script(MichelineSequence(), MichelineSequence())
            ) to "6d000ad2152600ac6bb16b5512e43a337dd562dc2cccfba3089a01fbb801e88a02ebca2e0000000001000000000100".asHexString().toByteArray(),
            OperationContent.Origination(
                "tz1LdF7qHCJg8Efa6Cx4LZrRPkvbh61H8tZq",
                "135675",
                "154",
                "23675",
                "34152",
                "763243",
                "tz1RY8er4ybXszZBbhtQDrYhA5AYY3VQXiKn",
                Script(MichelineSequence(), MichelineSequence())
            ) to "6d000ad2152600ac6bb16b5512e43a337dd562dc2cccfba3089a01fbb801e88a02ebca2eff0040b33c1a35d72f3c85747f605b1902d36fc8c9a300000001000000000100".asHexString().toByteArray(),
            OperationContent.Delegation(
                "tz1QVAraV1JDRsPikcqJVE4VccvW7vDWCJHy",
                "135675",
                "154",
                "23675",
                "34152",
            ) to "6e00352bb30ffdb72d101083a4fc5cd156f007705f5dfba3089a01fbb801e88a0200".asHexString().toByteArray(),
            OperationContent.Delegation(
                "tz1QVAraV1JDRsPikcqJVE4VccvW7vDWCJHy",
                "135675",
                "154",
                "23675",
                "34152",
                "tz1dStZpfk5bWsvYvuktDJgDEbpuqDc7ipvi",
            ) to "6e00352bb30ffdb72d101083a4fc5cd156f007705f5dfba3089a01fbb801e88a02ff00c356e7cb9943f6ef4168bea7915c7f88152e6c37".asHexString().toByteArray(),
            OperationContent.RegisterGlobalConstant(
                "tz1brHnNaHcpxqHDhqwmAXDq1i4F2A4Xaepz",
                "135675",
                "154",
                "23675",
                "34152",
                MichelineSequence(),
            ) to "6f00b1d399df432bbbdbd45cb6b454699ea96d77dabffba3089a01fbb801e88a020000000100".asHexString().toByteArray(),
            OperationContent.SetDepositsLimit(
                "tz1gxabEuUaCKk15qUKnhASJJoXhm9A7DVLM",
                "135675",
                "154",
                "23675",
                "34152",
            ) to "7000e9dcc1a4a82c49aeec327b15e9ed457dc22a1ebcfba3089a01fbb801e88a0200".asHexString().toByteArray(),
            OperationContent.SetDepositsLimit(
                "tz1gxabEuUaCKk15qUKnhASJJoXhm9A7DVLM",
                "135675",
                "154",
                "23675",
                "34152",
                "634",
            ) to "7000e9dcc1a4a82c49aeec327b15e9ed457dc22a1ebcfba3089a01fbb801e88a02fffa04".asHexString().toByteArray(),
        )

    private val invalidBytes: List<ByteArray>
        get() = listOf(
            "000000016cdaf9367e551995a670a5c642a9396290f8c9d17e6bc3c1555bfaa910d92214".asHexString().toByteArray(),
        )
}